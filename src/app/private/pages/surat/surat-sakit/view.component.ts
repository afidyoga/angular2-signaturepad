...
exportAll(){
	...
	if(this.tgl!=null){
		...
		this.suratExport.exportSuratSakit(param,"")
			.subscribe((resp: any) => {
				this.spinner.hide('spinner1')
			this.download(resp,`Surat Sakit.xlsx`)
		})
	}
}
export(data){
	...
	if(this.tgl!=null){
		param.date_from=this.convertTime(this.tgl[0])
		param.date_to=this.convertTime(this.tgl[1])
		this.suratExport.exportSuratSakit(param,data.id_diagnosa_surat_sakit)
			.subscribe((resp: any) => {
				this.spinner.hide('spinner1')
				let tgl=moment(new Date(data.tgl_antrian_unix)).format('DD/MM/YYYY')
			this.download(resp,`Surat Sakit-${data.nama}-${tgl}-${data.full_rm}.pdf`)
		})
	}
}
  convertDate(val){
  ...
  showDataTables() {
		...
		return {
			...
			ajax: (dataTablesParameters: any, callback: any) => {
				...
				this.surat.getSuratSakit(dataTablesParameters).subscribe((resp: any) => {
						this.spinner.hide('spinner1')
						callback({
