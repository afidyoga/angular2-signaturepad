...
export class AddComponent implements OnInit {
  ...
  hapusRetur(i,item,type){
    ...
    if(type=="obat")
    {
      ...
      let index=this.listPembelianObat.indexOf(item)
      this.satuan_qty_obat.splice(i,1)
      this.listPembelianObat[index+1].isDisable=false
    }
    else
    {
      this.listReturAlat.splice(i,1)
      this.satuan_qty_alat.splice(i,1)
      let index=this.listPembelianAlat.indexOf(item)
      this.listPembelianAlat[index+1].isDisable=false
    }
  }
