...
export class ViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.btnAdd =
      this.btnDelete =
      this.btnSetting =
        item.findIndex((val) => val.kode == "MGMKDK2") != -1 ? true : false;
    this.btnDetail = this.view =
      item.findIndex((val) => val.kode == "MGMKDK1") != -1 ? true : false;
    ...
    if (!this.view) {
      Swal.fire(
        "Warning",
        "Anda tidak mempunyai akses halaman ini",
        "warning"
      ).then(() => {
  ...
  SubmitForm() {
    ...
    payload.latitude = this.formTambah.value.latitude;
    payload.longitude = this.formTambah.value.longitude;
    ...
    payload.status_pcare =
      this.formTambah.value.status_pcare == null
        ? "0"
        : String(Number(this.formTambah.value.status_pcare));
    payload.status_satuSehat =
      this.formTambah.value.status_satusehat == null
        ? "0"
        : String(Number(this.formTambah.value.status_satusehat));
  ...
  prosesSelectProvinsi(event: any, aksi: string, tipe: any) {
    if (aksi == "search") {
      ...
      if (
        ...
        this.paramProvinsiKtp.search.length >= 3
      ) {
    ...
    if (aksi == "last_page") {
      if (!this.isLastProv) this.paramProvinsiKtp.last_data += 10;
    }
    if (!this.isLastProv) {
      this.loadingSelectProv = true;
      this.wilayahService
        .prosesSelectOptionProvinsi(this.paramProvinsiKtp, aksi)
        .subscribe((res) => {
          this.loadingSelectProv = false;
          if (res) {
            if (res.response.length == 0) this.isLastProv = true;
            else {
              if (res.response.length < 10) {
                ...
                res.response.map((val) => {
                  ...
                  if (i == -1) this.listProvinsiKtp.push(val);
                });
                this.isLastProv = true;
              } else {
                res.response.map((val) => {
  ...
  prosesSelectZone(event: any, aksi: string) {
    ...
    if (aksi == "last_page") {
      if (!this.isLastZone) this.paramZone.last_data += 10;
    }
    if (!this.isLastZone) {
        ...
        if (res) {
          if (res.response.length == 0) {
            ...
            if (res.response.length < 10) {
              ...
              res.response.map((val) => {
                let i = this.listZone.findIndex(
                  (x) => x.id_time_zone == val.id_time_zone
                );
                if (i == -1) this.listZone.push(val);
              });
              this.isLastZone = true;
            } else {
              res.response.map((val) => {
  ...
  prosesSelectKabupaten(event: any, aksi: string, tipe: any) {
    ...
    if (aksi == "search") {
      ...
      if (
        ...
        this.paramKabupatenKtp.search.length >= 3
      ) {
        this.listKabupatenKtp = [];
        this.listKecamatanKtp = [];
        this.listDesaKtp = [];
    ...
    if (aksi == "last_page") {
      if (!this.isLastKab) this.paramKabupatenKtp.last_data += 10;
    }
    if (!this.isLastKab) {
      this.loadingSelectKab = true;
      this.wilayahService
        .prosesSelectOptionKabupaten(this.paramKabupatenKtp, aksi, idProvinsi)
        .subscribe((res) => {
          ...
          if (res) {
            if (res.response.length == 0) {
              ...
              if (res.response.length < 10) {
                ...
                res.response.map((val) => {
                  let i = this.listKabupatenKtp.findIndex(
                    (x) => x.id == val.id
                  );
                  if (i == -1) this.listKabupatenKtp.push(val);
                });
                this.isLastKab = true;
              } else {
                res.response.map((val) => {
  ...
  prosesSelectKecamatan(event: any, aksi: string, tipe: any) {
    ...
    if (aksi == "search") {
      ...
      if (
        ...
        this.paramKecamatanKtp.search.length >= 3
      ) {
        this.listKecamatanKtp = [];
        this.listDesaKtp = [];
        this.paramKecamatanKtp.last_data = 0;
    ...
    if (aksi == "last_page") {
      if (!this.isLastKec) this.paramKecamatanKtp.last_data += 10;
    }
    if (!this.isLastKec) {
      this.loadingSelectKec = true;
      this.wilayahService
        .prosesSelectOptionKecamatan(this.paramKecamatanKtp, aksi, idKabupaten)
        .subscribe((res) => {
          this.loadingSelectKec = false;
          if (res) {
            if (res.response.length == 0) this.isLastKec = true;
            else {
              if (res.response.length < 10) {
                if (aksi == "search") this.listKecamatanKtp = [];
                res.response.map((val) => {
                  let i = this.listKecamatanKtp.findIndex(
                    (x) => x.id == val.id
                  );
                  if (i == -1) this.listKecamatanKtp.push(val);
                });
                this.isLastKec = true;
              } else {
                res.response.map((val) => {
  ...
  prosesSelectDesa(event: any, aksi: string, tipe: any) {
    ...
    if (aksi == "search") {
      ...
      if (
        ...
        this.paramDesaKtp.search.length >= 3
      ) {
        this.listDesaKtp = [];
        this.paramDesaKtp.last_data = 0;
        this.isLastDes = false;
    ...
    if (aksi == "last_page") {
      if (!this.isLastDes) this.paramDesaKtp.last_data += 10;
    }
    if (!this.isLastDes) {
      this.loadingSelectDes = true;
      this.wilayahService
        .prosesSelectOptionDesa(this.paramDesaKtp, aksi, idKecamatan)
        .subscribe((res) => {
          this.loadingSelectDes = false;
          if (res) {
            if (res.response.length == 0) this.isLastDes = true;
            else {
              if (res.response.length < 10) {
                if (aksi == "search") this.listDesaKtp = [];
                res.response.map((val) => {
                  let i = this.listDesaKtp.findIndex((x) => x.id == val.id);
                  if (i == -1) this.listDesaKtp.push(val);
                });
                this.isLastDes = true;
              } else {
                res.response.map((val) => {
      ...
      .subscribe(async (resp: any) => {
        if (resp) {
          ...
          this.formTambah.patchValue({
            ...
            latitude: this.daftarKlinik.latitude,
            longitude: this.daftarKlinik.longitude,
            ...
            organization_id: this.daftarKlinik.organization_id,
            client_id: this.daftarKlinik.client_id,
            client_secret: this.daftarKlinik.client_secret,
            status_satusehat: Number(this.daftarKlinik.status_satusehat),
          });
          this.listProvinsiKtp = [
            { id: resp.response.id_province, name: resp.response.provinsi },
          ];
          this.listKecamatanKtp = [
            { id: resp.response.id_district, name: resp.response.kecamatan },
          ];
          this.listKabupatenKtp = [
            { id: resp.response.id_regency, name: resp.response.kabupaten },
          ];
          this.listDesaKtp = [
            { id: resp.response.id_village, name: resp.response.desa },
          ];
          ...
          this.timeZone = this.daftarKlinik.config_time_zone;
          this.spinner.hide("spinner1");
          this.modalService.open("modalFormContent");
        }
      });
  ...
  detail(data) {
    this.router.navigate([
      "manajemen-klinik/informasi-klinik/view/" + data.id_klinik,
    ]);
  }
  nonAktif(data: any) {
    Swal.fire({
      ...
      if (result.value) {
        this.store.dispatch(
          DaftarKlinikActions.deleteDaftarKlinik({
            payload: { id: data.id_klinik },
          })
        );
        setTimeout(() => {
          this.reLoadData();
  ...
  showDataTables(settings, detail, del) {
    ...
    return {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.daftarKlinikService
          .getDataTables(dataTablesParameters)
          .subscribe((resp: any) => {
            callback({
