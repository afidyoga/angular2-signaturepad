...
export class TambahComponent implements OnInit, AfterViewInit {
	...
	changeRoleType(event: any) {
		/**
		this.typeRoleService.show(event.target.value)
		.subscribe(succ => {
		  this.dataMenu = this.typeRoleService.functionFilterJson(succ.response.detail)
		})
		*/
		this.roleService.getMenuRight(event.target.value)
			.subscribe(succ => {
				// this.dataMenu = this.typeRoleService.functionFilterJson(succ.response)
