...
import { Sweep } from '@ngrx/store-devtools/src/actions';
@Component({
	selector: 'app-add',
...
export class AddComponent implements OnInit {
			...
			.subscribe((resp: any) => {
				if(resp){
					if(this.optionSupplier.length==0){
						this.optionSupplier=resp.response
					}else{
						resp.response.map(val=>{
							let index=this.optionSupplier.findIndex(x=>x.id_supplier==val.id_supplier)
							if(index<0){
	...
	prosesSelectAlat(event: any, aksi: string) {
		...
		this.alatKesehatanService.get(this.params_get)
			.subscribe((resp: any) => {
				if(resp){
					if(this.optionAlatKesehatan.length==0){
						this.optionAlatKesehatan=resp.response
					}else{
						resp.response.map(val=>{
							let index=this.optionAlatKesehatan.findIndex(x=>x.id_alat_kesehatan==val.id_alat_kesehatan)
							if(index<0){
