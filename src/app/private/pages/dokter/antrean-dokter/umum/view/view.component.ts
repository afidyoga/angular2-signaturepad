...
import { AntreanUmumPayload, TindakanMedisPayload, AssessmentPayload, KunjunganTerakhir, Perawat, DokterUmum } from 'src/app/private/models/class-payload-api/dokter/antrean-dokter/antrean-umum-payload';
...
export class ViewComponent implements OnInit {
	...
	constructor(
		...
		private tindakanPerawat:TindakanMedisService,
	...
	prosesSelectAssesment(event: any, aksi: string) {
		...
		if(aksi=="last_page"){
			let last=this.paramAssesment.last_data+this.paramAssesment.get_data
			this.paramAssesment.last_data=last
		}else{
			this.paramAssesment.last_data=0
		}
		this.assessmentService.prosesSelectAssesment(param, aksi)
			.subscribe(resp => {
				...
				if(resp){
					if(this.listAssesment.length==0){
						this.listAssesment=resp.response
					}else{
						resp.response.map(val=>{
							let index=this.listAssesment.findIndex(x=>x.id_assessment==val.id_assessment)
							if(index<0){
	...
	prosesSelectTindakanMedis(event: any, aksi: string) {
		...
		if(aksi=="last_page"){
			let last=this.paramTindakanMedis.last_data+this.paramTindakanMedis.get_data
			this.paramTindakanMedis.last_data=last
		}else{
			this.paramTindakanMedis.last_data=0
		}
		this.tindakanMedisUmumService.prosesSelectOption(param, aksi)
			.subscribe(resp => {
				...
				if(resp){
					if(this.listTindakanMedis.length==0){
						this.listTindakanMedis=resp.response
					}else{
						resp.response.map(val=>{
