...
import { DataPasienService } from 'src/app/private/modul-api/modul-resepsionis/pasien.service'
...
export class ViewComponent implements OnInit {
	...
	constructor(
		...
		private pasienService:DataPasienService,
	...
	// 	this.pasienService.getSelectOption(this.params_get)
	// 		.subscribe((resp: any) => {
	// 			this.optionPasien= resp.response
	// 		})
	// }
	prosesSelectSesi(event: any, aksi: string) {
		...
		this.sesiService.getSelectOption(this.params_get)
			.subscribe((resp: any) => {
				if(resp){
					if(this.optionSesi.length==0){
						this.optionSesi=resp.response
					}else{
			...
			columns: [
				...
				},
				{
					...
					render(data: any, type: any, row: any, full: any) {
						...
						return det?`<button class="btn btn-sm btn-success add mb-2" ${add}>Add Resep</button>
						<button class="btn btn-sm btn-info detail mb-2" ${finis}>Detail</button>
						<button class="btn btn-sm btn-warning cetak mb-2" ${finis}>Cetak</button>`:'';
					}
				}
			],
