...
export class AESService {
    ...
    tgl=new Date().getTime()
    iv=(Math.random() + '').substring(2,10) + (Math.random() + '').substring(2,10);
    iv2=(Math.random() + '').substring(2,10)  + (Math.random() + '').substring(2,10);

    keyGen=this.tgl+new Date(moment.utc(new Date(this.tgl)).format('YYYY-MM')+'-01').getTime()
